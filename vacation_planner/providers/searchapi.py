...
def _is_quota(status: int, body: dict | None) -> bool:
    # Only the exhausted plan, not every message mentioning a "limit": a rate limit is
    # transient and must stay retryable instead of killing the provider for the whole run.
    msg = ((body or {}).get("error") or "").lower()
    return status in (402, 429) or "credit" in msg or "out of" in msg
...
class SearchApiClient:
    ...
    def params_for(self, req: SearchRequest) -> dict[str, str]:
        p = {
            ...
            "stops": STOPS.get(self.settings.max_stops, "any"), "api_key": self.api_key,
        }
    ...
    def search(self, req: SearchRequest, raw_dir: Path | None = None) -> SearchResult:
        ...
        try:
            ...
            raise ProviderError(f"searchapi: parse failed: {type(e).__name__}: {e}") from e
